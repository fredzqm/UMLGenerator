...
public class DesignParser {
	...
	private static void printFields(ClassNode classNode) {
		...
		for (FieldNode field : fields) {
			...
			System.out.print("	final? " + ((field.access & Opcodes.ACC_FINAL) > 0));
			System.out.print("	public? " + ((field.access & Opcodes.ACC_PUBLIC) > 0));
			System.out.println("	static? " + ((field.access & Opcodes.ACC_STATIC) > 0));
	...
	private static void printMethods(ClassNode classNode) {
		...
		for (MethodNode method : methods) {
			...
			// Query the access modifiers with the ACC_* constants.
			System.out.print("	public? " + ((method.access & Opcodes.ACC_PUBLIC) > 0));
			System.out.print("	abstract? " + ((method.access & Opcodes.ACC_ABSTRACT) > 0));
