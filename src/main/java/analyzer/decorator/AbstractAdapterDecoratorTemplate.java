...
public abstract class AbstractAdapterDecoratorTemplate implements IAnalyzer {
     ...
     */
    protected Collection<IClassModel> updateRelatedClasses(Collection<IClassModel> updatedClasses, IClassModel clazz) {
        return updatedClasses;
    }

    private Collection<? extends IClassModel> updateClasses(Map<IClassModel, Collection<IClassModel>> updateMap) {
        Collection<IClassModel> updatedClasses = new LinkedList<>(); // We are only inserting. Avoid worst case.
